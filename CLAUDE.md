...
**Failed OCR is never a dead end, and it never costs the user their photos.** Steps 2 and 3 share one `OcrFallback` component offering two ways out: *Retry verification*, which calls `POST /user/verification/:kind/retry-ocr` to re-read the scan **already stored server-side**, or *Enter the number manually*, which reveals the number field in that same step. Neither clears the captured images — OCR fails for reasons that have nothing to do with the photo (provider timeout, an IP off the allowlist), so re-photographing a document that was fine is the wrong response to our outage. This was the first design and it was wrong; don't reintroduce it.

**The licence step mirrors the Aadhaar step exactly**: `POST /user/verification/licence/scan` then `POST /user/verification/licence/number` only when OCR couldn't read it.
...
**Documents can come from the camera OR the gallery; the selfie is camera-only.** `captureDocument()` offers both via an Alert; `captureSelfie()` is `launchCamera` with `cameraType: 'front'`. Liveness is the selfie's requirement — a licence photographed last week reads just as well, and forcing the camera there turns a blocked permission into a blocked signup.

**Never pre-check camera permission — this was a real bug.** `requestCameraPermission()` in `utils/utils.js` is a **diagnostic**, consulted only AFTER the picker fails with a permission `errorCode`, to decide whether to prompt again or send the user to Settings. It must never gate a capture: `react-native-permissions` returns `UNAVAILABLE` for any permission whose native handler isn't linked into the build, and the iOS Camera handler is opt-in via `setup_permissions` in `ios/Podfile` — where it was commented out. A pre-check therefore reported "permission error" on phones whose camera worked and whose permission was already granted. The Podfile line is now enabled (needs `pod install`), but the code deliberately does not depend on it, so older builds still work. `UNAVAILABLE` is surfaced as `'unknown'` precisely so no caller can mistake it for a hardware verdict.
