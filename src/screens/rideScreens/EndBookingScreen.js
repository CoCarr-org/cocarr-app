...
export function EndBookingScreen({route}) {
  ...
  const selectImage = async (type) => {
    try {
      ...
      launchCamera({ mediaType: 'photo', selectionLimit: 1, cameraType: 'back',quality:0.5}, (response) => {
  ...
  const onSubmit = async () => {
    try {
      ...
      const response = await axios.post(`${API_URL}/booking/end-ride/${booking.id}`, {
        endDateTime: new Date().toISOString(),
        endKms: data.endKms,
        endOtp: data.endOtp,
        endImages: uploadedImages
      });
    ...
    <View style={{flex:1,backgroundColor:'#000',height:'100%'}}>
        ...
        <ScrollView style={{flex:1,}}>
          ...
          <CustomText fontType='primary' weight='SemiBold' style={{color:'#757575', fontSize:11,marginBottom:4,textAlign:'left',textTransform:'uppercase'}}>End Odometer Reading</CustomText>
            <TextInput
            style={{backgroundColor:'#1c1c1e',padding:10,borderRadius:5,textAlign:'left',marginBottom:18,color:'#a3a3a3'}}
            value={data.endKms}
            placeholderTextColor='#757575'
            onChangeText={(value)=>setData({...data,endKms:value})}
            />

<CustomText fontType='primary' weight='SemiBold' style={{color:'#757575', fontSize:11,marginBottom:4,textTransform:'uppercase',textAlign:'center'}}>End Ride OTP</CustomText>
            <View style={{flexDirection: 'row', justifyContent: 'center', marginBottom: 18}}>
              <OtpInput length={6} value={data.endOtp} onChange={(value)=>setData({...data,endOtp:value})}/>
            </View>
        ...
        <TouchableOpacity disabled={isLoading || !data.endDateTime || !data.endKms || !data.endOtp || !data.images.front || !data.images.back || !data.images.driverSide || !data.images.passengerSide || !data.images.fuelOdometer || submitLoading} onPress={onSubmit} style={{backgroundColor:(isLoading || !data.endDateTime || !data.endKms || !data.endOtp || !data.images.front || !data.images.back || !data.images.driverSide || !data.images.passengerSide || !data.images.fuelOdometer || submitLoading) ? '#4C4C4E' : BRAND_COLOR,borderRadius:5,paddingVertical:12,paddingHorizontal:12,color:'#fff',fontSize:14,width:'100%',marginTop:12,justifyContent:'center',alignItems:'center'}}>
